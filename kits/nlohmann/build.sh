...
#
JSON_SRC_PATH=${SHELL_PATH}/json-3.11.2/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/include/nlohmann/json.hpp ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/nlohmann/json/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${JSON_SRC_PATH} \
    ...
    exit_if_error $? "json配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "json编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "json安装错误。" 1
