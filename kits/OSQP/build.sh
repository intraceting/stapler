...
#
OSQP_SRC_PATH=${SHELL_PATH}/osqp-20240116/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libosqp.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/osqp/
    #删除过时的配置。
    ...
    #复制源码到临时目录。
    cp -rf ${OSQP_SRC_PATH}/* ${BUILD_TMP_PATH}/
    ...
    exit_if_error $? "osqp配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "osqp编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "osqp安装错误。" 1
