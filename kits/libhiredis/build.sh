...
#
LIBHIREDIS_SRC_PATH=${SHELL_PATH}/hiredis-1.1.0/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libhiredis.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/libhiredis/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${LIBHIREDIS_SRC_PATH} \
        ...
        -DENABLE_SSL=ON
    exit_if_error $? "libhiredis配置错误。" 1

    #编译。
    make -s -j4
    exit_if_error $? "libhiredis编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "libhiredis安装错误。" 1
