...
#
NGHTTP2_SRC_PATH=${SHELL_PATH}/nghttp2-1.55.0/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libnghttp2.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/nghttp2/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${NGHTTP2_SRC_PATH} \
        ...
        -DENABLE_LIB_ONLY=ON
    exit_if_error $? "nghttp2配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "nghttp2编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "nghttp2安装错误。" 1
