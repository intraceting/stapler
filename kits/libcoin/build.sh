...
#检查是否已经创建。
if [ ! -f ${TARGET_PERFIX_PATH}/lib/libcoin.so ];then
{
    #临时目录。
    ...
    #执行配置。
    ${NATIVE_PERFIX_PATH}/bin/cmake ${LIBCOIN_SRC_PATH} \
    ...
    exit_if_error $? "libcoin配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "libcoin编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "libcoin安装错误。" 1
