...
#
ZLMEDIAKIT_SRC_PATH=${SHELL_PATH}/ZLMediaKit/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/bin/MediaServer ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/ZLMediaKit/
    #删除过时的配置。
    rm -rf ${BUILD_TMP_PATH}
    #生成临时目录。
    mkdir -p ${BUILD_TMP_PATH}/
    #复制源码到临时目录。
    cp -rf ${ZLMEDIAKIT_SRC_PATH}/* ${BUILD_TMP_PATH}/
    #生成临时目录。
    ...
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${BUILD_TMP_PATH}/ \
        ...
        -DENABLE_WEBRTC=OFF
    exit_if_error $? "ZLMediaKit配置错误。" 1

    #编译。
    make -s -j4
    exit_if_error $? "ZLMediaKit编译错误。" 1

     #安装。
    make install 
    exit_if_error $? "ZLMediaKit安装错误。" 1
