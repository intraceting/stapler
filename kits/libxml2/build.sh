...
#
LIBXML2_SRC_PATH=${SHELL_PATH}/libxml2-20240119/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libxml2.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/libxml2/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${LIBXML2_SRC_PATH} \
        ...
        -DCMAKE_C_FLAGS="-O3 -L${STAPLER_TARGET_PREFIX_PATH}/lib -l:libz.so.1" \
        -DCMAKE_CXX_FLAGS="-O3 -L${STAPLER_TARGET_PREFIX_PATH}/lib -l:libz.so.1" \
        -DCMAKE_BUILD_TYPE=Release \
        -DLIBXML2_WITH_ICONV=OFF \
        -DLIBXML2_WITH_LZMA=OFF \
        -DLIBXML2_WITH_PYTHON=OFF \
        -DLIBXML2_WITH_TESTS=OFF
    exit_if_error $? "libxml2配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "libxml2编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "libxml2安装错误。" 1
