...
#检查是否已经创建。
if [ ! -f ${TARGET_PREFIX_PATH}/lib/libfreetype.so ] && [ ! -f ${TARGET_PREFIX_PATH}/lib64/libfreetype.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${BUILD_PATH}/freetype/
    #删除过时的配置。
    ...
    #执行配置。
    ${NATIVE_PREFIX_PATH}/bin/cmake ${FREETYPE_SRC_PATH} \
