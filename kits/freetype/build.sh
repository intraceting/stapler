...
#7：生成目标平台的FREETYPE。
FREETYPE_SRC_PATH=${SHELL_PATH}/freetype-2.13.0/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libfreetype.so ] && [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib64/libfreetype.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/freetype/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${FREETYPE_SRC_PATH} \
    ...
    exit_if_error $? "freetype配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "freetype编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "freetype安装错误。" 1
