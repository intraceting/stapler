...
#检查是否已经创建。
if [ ! -f ${TARGET_PERFIX_PATH}/lib/libfaiss.so ] && [ ! -f ${TARGET_PERFIX_PATH}/lib64/libfaiss.so ];then
{
    #临时目录。
    ...
    #执行配置。
    ${NATIVE_PERFIX_PATH}/bin/cmake ${FAISS_SRC_PATH} \
        ...
        -DMKL_LIBRARIES=${TARGET_PERFIX_PATH}/lib/libopenblas.so
