...
#
BLOSC_SRC_PATH=${SHELL_PATH}/c-blosc-1.21.0/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libblosc.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/blosc/
    #删除过时的配置。
    rm -rf ${BUILD_TMP_PATH}
    #生成临时目录。
    mkdir -p ${BUILD_TMP_PATH}/
    #复制源码到临时目录。
    cp -rf ${BLOSC_SRC_PATH}/* ${BUILD_TMP_PATH}/
    #生成临时目录。
    ...
    exit_if_error $? "BLOSC配置错误。" 1

    #编译。
    make -s -j4
    exit_if_error $? "BLOSC编译错误。" 1

     #安装。
    make install 
    exit_if_error $? "BLOSC安装错误。" 1
