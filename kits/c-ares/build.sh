...
#
ARES_SRC_PATH=${SHELL_PATH}/c-ares-1.19.1/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libcares.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/ares/
    #删除过时的配置。
    ...
    cd ${BUILD_TMP_PATH}/
     ...
     #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${ARES_SRC_PATH} \
    ...
    exit_if_error $? "ares配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "ares编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "ares安装错误。" 1
