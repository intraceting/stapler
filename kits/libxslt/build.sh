...
#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libxslt.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/libxslt/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${LIBXSLT_SRC_PATH} \
    ...
    exit_if_error $? "libxslt配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "libxslt编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "libxslt安装错误。" 1
