...
#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/include/cimg/CImg.h ];then
{
    #创建安装目录。
    mkdir -p ${STAPLER_TARGET_PREFIX_PATH}/include/cimg/
    #不需要编译，直接复制源码到安装目录。
    cp -rf ${CIMG_SRC_PATH}/CImg.h ${STAPLER_TARGET_PREFIX_PATH}/include/cimg/
}
fi