...
#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libcnpy.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/cnpy-20231107/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${CNPY_SRC_PATH}/ \
    ...
    exit_if_error $? "cnpy配置错误。" 1

    #编译。
    make -s -j4
    exit_if_error $? "cnpy编译错误。" 1

     #安装。
    make install 
    exit_if_error $? "cnpy安装错误。" 1
