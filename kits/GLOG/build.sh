...
#
GLOG_SRC_PATH=${SHELL_PATH}/glog-0.6.0/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libglog.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/glog/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${GLOG_SRC_PATH}/ \
    ...
    exit_if_error $? "glog配置错误。" 1

    #编译。
    make -s -j4
    exit_if_error $? "glog编译错误。" 1

     #安装。
    make install 
    exit_if_error $? "glog安装错误。" 1
