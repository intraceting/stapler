...
#
BENTO4_SRC_PATH=${SHELL_PATH}/Bento4-1.6.0-640/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libap4.a ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/Bento4/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${BENTO4_SRC_PATH} \
    ...
    exit_if_error $? "Bento4配置错误。" 1


    #编译。
    make -s -j4
    exit_if_error $? "Bento4编译错误。" 1

    #安装。
    make install 
    exit_if_error $? "Bento4安装错误。" 1
