...
#
GFLAGS_SRC_PATH=${SHELL_PATH}/gflags-2.2.2/

#检查是否已经创建。
if [ ! -f ${STAPLER_TARGET_PREFIX_PATH}/lib/libgflags.so ];then
{
    #临时目录。
    BUILD_TMP_PATH=${STAPLER_BUILD_PATH}/gflags/
    #删除过时的配置。
    ...
    #执行配置。
    ${STAPLER_NATIVE_PREFIX_PATH}/bin/cmake ${GFLAGS_SRC_PATH}/ \
        ...
        -DINSTALL_HEADERS=ON
    exit_if_error $? "gflags配置错误。" 1

    #编译。
    make -s -j4
    exit_if_error $? "gflags编译错误。" 1

     #安装。
    make install 
    exit_if_error $? "gflags安装错误。" 1
