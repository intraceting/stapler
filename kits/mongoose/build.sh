...
#检查是否已经创建。
if [ ! -f ${TARGET_PERFIX_PATH}/lib/libmongoose.so ];then
{
    #临时目录。
    ...
    #编译和安装。
    make -s -j4 install PREFIX=${TARGET_PERFIX_PATH}/ SSL=OPENSSL OPENSSL=${TARGET_PERFIX_PATH}/ CC=${TARGET_COMPILER_C} AR=${TARGET_COMPILER_AR} "CFLAGS=-fPIC"
